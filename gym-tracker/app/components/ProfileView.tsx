...
const VALID_STAT_IDS = new Set<StatId>(ALL_STAT_DEFS.map((d) => d.id));

function friendStatsStorageKey(friendUserId: string) {
  return `@gym_tracker_friend_stats_${friendUserId}`;
}

function parseProfileStats(value: unknown): StatId[] | null {
  if (!Array.isArray(value)) return null;
  const ids = value.filter((id): id is StatId => typeof id === "string" && VALID_STAT_IDS.has(id as StatId));
  if (ids.length === 0) return null;
  return ids.slice(0, 3);
}

async function loadStatsFromStorage(key: string): Promise<StatId[] | null> {
  const val = await AsyncStorage.getItem(key);
  if (!val) return null;
  try {
    return parseProfileStats(JSON.parse(val));
  } catch {
    return null;
  }
}
...
export default function ProfileView({ userId: userIdProp, readOnly = false, showBack = false, onBack }: ProfileViewProps) {
  ...
  const loadProfile = useCallback(async (isRefresh = false) => {
    ...
    const savedStatsPromise = loadStatsFromStorage(STATS_STORAGE_KEY);

    try {
      const { data: { user } } = await db.getUser();
      if (!user) { setError("Not authenticated"); return; }

      const targetUserId = userIdProp ?? user.id;
      if (readOnly) {
        const { data: allowed, error: accessErr } = await db.canViewFriendProfile(user.id, targetUserId);
        if (accessErr) { setError(accessErr.message); return; }
        if (!allowed) { setError("You do not have access to this profile."); return; }
      }

      const [profileResult, sessionsResult, localStatIds, cardioStatsResult] = await Promise.all([
        db.getUserProfile(targetUserId),
        db.getWorkoutSessions(targetUserId),
        savedStatsPromise,
        db.getCardioStats(targetUserId),
      ]);
      ...
      const profileStats = parseProfileStats(profileResult.data?.profile_stats);
      let resolvedStats = DEFAULT_STATS;

      if (readOnly && userIdProp) {
        const viewerOverride = await loadStatsFromStorage(friendStatsStorageKey(userIdProp));
        resolvedStats = viewerOverride ?? profileStats ?? DEFAULT_STATS;
      } else {
        resolvedStats = profileStats ?? localStatIds ?? DEFAULT_STATS;
        await AsyncStorage.setItem(STATS_STORAGE_KEY, JSON.stringify(resolvedStats));
        if (!profileStats && localStatIds && user.id) {
          db.updateUserProfile(user.id, { profile_stats: localStatIds }).catch(() => {});
        }
      }
      setSelectedStats(resolvedStats);
  ...
  const handleSaveStats = useCallback(async (ids: StatId[]) => {
    ...
    if (readOnly && userIdProp) {
      await AsyncStorage.setItem(friendStatsStorageKey(userIdProp), JSON.stringify(ids));
    } else {
      await AsyncStorage.setItem(STATS_STORAGE_KEY, JSON.stringify(ids));
      const { data: { user } } = await db.getUser();
      if (user?.id) {
        await db.updateUserProfile(user.id, { profile_stats: ids });
      }
    }
  ...
  const listHeader = (
    <>
      {/* ── Top bar ── */}
      <View style={[styles.topBar, { borderBottomColor: colors.border, paddingTop: showBack ? insets.top + 8 : 60 }]}>
        {showBack ? (
          <Pressable onPress={onBack ?? (() => router.back())} style={styles.settingsBtn} hitSlop={8}>
            <Ionicons name="chevron-back" size={26} color={colors.primary} />
          </Pressable>
        ) : null}
        <Text style={[styles.username, { color: colors.text }, showBack && { textAlign: "center" }]}>
        ...
        renderItem={({ item }) => (
          <View style={{ marginHorizontal: 16 }}>
            <WorkoutHistoryCard
              ...
              onEditWorkout={readOnly ? undefined : handleEditWorkout}
              readOnly={readOnly}
      ...
      <Pressable style={styles.overlay} onPress={onClose}>
        <Pressable style={[styles.sheet, { backgroundColor: colors.surface }]} onPress={() => {}}>
          ...
          <Text style={[styles.sheetSubtitle, { color: colors.textSecondary }]}>
            {readOnlyContext
              ? "Pick up to 3 stats to show on this friend's profile (saved on your device)"
              : `Pick up to 3 stats to show on your profile${atMax ? " — deselect one to swap" : ""}`}
          </Text>
